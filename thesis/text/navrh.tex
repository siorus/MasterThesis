...
\vspace{1em}
\noindent
Druhým nástrojom s~podobnými vlastnosťami a nedostatkami je aj \texttt{Router Auditing Tool} \cite{OniomAfGpef53LHq}, ktorý má naviac aj \zkratka{zkGUI}. Existuje niekoľko rozšírení aj pre nástroj \texttt{Nessus}, ktoré overujú dodržiavanie odporúčaní a podľa zistení čerpajú z~CIS Benchmarku \cite{CIS_DrTLsgXv24lxeIIM}. Taktiež však nepodporujú zjednanie nápravy a ignorujú umiestnenie zariadenia v~topológii.
...
\end{minipage}
	...
	\item \vspace{2em} Viacnásobný výskyt v rôznych kontextoch\,--\,tieto príkazy konfigurujú rôzne služby, napríklad autentifikáciu správ \zkratka{zkOSPF} alebo prístup k manažmentu zariadenia.
...
{\small
	...
	\dirtree{%.
		...
		.5 device\_info.yaml\DTcomment{šalóna na uloženie základných informácií o~zaridení}.
		.5 yaml\_module\_configs.
		.6 01\_01\_aaa\_new\_model.yaml\DTcomment{šablóna konfiguračného súboru k~modulu}.
		.6 11\_02\_cdp\_enable.yaml\DTcomment{šablóna konfiguračného súboru k~modulu}.
		.5 python\_modules.
		.6 device\_info.py\DTcomment{Python modul/trieda na uloženie informácií o zariadení a prácu s device\_info.yaml}.
		.5 modules\_by\_facility\_layer.yaml\DTcomment{šablóna s~definíciou, ktoré moduly  budú spustené pre zariadenia na konkrétnej vrstve modelu}.
		.3 hp.
		.3 juniper.
		.4 junos.
		.3 device\_info\_abstract.py\DTcomment{abstraktná trieda z ktorej dedia všetky nové device\_info.py}.
		.3 own\_variables.yaml\DTcomment{šablóna pre premenné nutné pri generovaní nápravy}.
		.3 yaml\_module.py\DTcomment{Python modul/trieda na prácu s YAML konfiguračnými modulmi}.
		...
		.4 current\_fixes\DTcomment{adresár s nápravami}.
		.5 hostname1\_fix.txt.
		.5 hostname2\_fix.txt.
		.4 reports\DTcomment{adresár so správami}.
		.5 hostname1\_report.html\DTcomment{detailnejšia správa pre konkrétne zariadenie}.
		.5 hostname2\_report.html.
		.5 summary\_report.html\DTcomment{správa s~mapovaním nedostatok-zariadenie}.
		.4 hostname1\DTcomment{zložka pre jedno konkrétne zariadenie}.
		.5 device\_info.yaml\DTcomment{základané informácie o~konkrétnom zariadení}.
		.5 01\_01\_aaa\_new\_model.yaml\DTcomment{konfiguračný súbor k~modulu s~nájdenými nedostatkami}.
		.5 11\_02\_cdp\_enable.yaml.
		.5 hostname1\_config.txt\DTcomment{aktuálne skenovaná konfigurácia}.
		.4 hostname2.
		.4 own\_variables.yaml\DTcomment{súbor s premennými nutnými pri generovaní nápravy}.
		.4 modules\_by\_facility\_layer.yaml\DTcomment{súbor s~definíciou, ktoré moduly  budú spustené pre zariadenia na konkrétnej vrstve modelu}.
		.3 workspace2.
		...
		.3 workspace2.
		.2 old\_configs\DTcomment{adresár s~predchádzajúcimi nálezmi a nápravami z rovnakého workspace}.
		.3 workspace1.
		.2 report\DTcomment{adresár s CSS a HTMLpre vytvorenie správy}.
		.3 report.css.
		.3 report\_begin.html.
		.3 report\_end.html.
	}
}
...
%Podrobná tabuľka s~mapovaním odporúčaní na príkazy pre zariadenia Cisco s~operačným systémom IOS je v~prílohe \ref{apendix:cisco_cmd}.
...
\begin{longtable}[!htbp]{|C{2.5em}L{11em}L{11em}C{2.5em}C{4em}C{5em}L{2.5em}|}
	...
	\rowcolor[rgb]{ .94,  .94,  .94} 1	&IP spoofing	&Špecifikácia ACL na zakázanie a logovanie privátnych a špeciálnych IP adries z~RFC 1918, RFC 3330, RFC 6890, RFC 8190	&C	&C	&CE,
	CCD,
...
\begin{longtable}[!htbp]{|C{2.5em}L{11em}L{11em}C{2.5em}C{4em}C{5em}L{2.5em}|}
	...
	\cite{uYLsMtQInofenpV3}\\
	\rowcolor[rgb]{ .94,  .94,  .94} 3	&Vysoké zaťaženie CPU	&Nastavenie notifikácie pri vysokom zaťažení CPU	&M	&M	&A	& \cite{Singh2018}
...
\begin{longtable}[!htbp]{|C{2.5em}L{11em}L{11em}C{2.5em}C{4em}C{5em}L{2.5em}|}
	...
	\rowcolor[rgb]{ .94,  .94,  .94} 3	&Nemožnosť identifikovať pravidlo v~ACL	&Popis každého pravidla v~ACL pre lepšiu identifikáciu	&M	&L	&A	& \cite{Singh2018}\\
...
\begin{longtable}[!htbp]{|C{2.5em}L{11em}L{11em}C{2.5em}C{4em}C{5em}L{2.5em}|}
	...
	\cite{Akin2002}\\
	\rowcolor[rgb]{ .94,  .94,  .94} 5	&Odpočúvanie SNMP sedenia z~dôvodu slabého šifrovania a hashovacej  funkcie	&Vytvorenie SNMP verzie 3 užívateľa s~minimálnym šifrovaním AES 128 bit a hashovacou funkciou SHA	&M	&C	&A	& \cite{Barker2019}
...
\begin{longtable}[!htbp]{|C{2.5em}L{11em}L{11em}C{2.5em}C{4em}C{5em}L{2.5em}|}
	...
	\rowcolor[rgb]{ .94,  .94,  .94} 1	&Rogue root bridge	& Rogue root bridge protection (root guard)	&C	&C	&DI,
	CDA,
	AC	& \cite{Vyncke2008}\\
	2	&Pripojenie prepínaču na koncový prístupový port	& BPDU protection (BPDU guard)	&C	&C	&DI,
	CDA,
	AC	& \cite{Vyncke2008}\\
	\rowcolor[rgb]{ .94,  .94,  .94} 3	&Rýchlosť konvergencie	&Prístupové porty by sa nemali podieľať na STP procese	&C	&H	&DI,
	CDA,
	AC	& \cite{Vyncke2008}\\
...
\begin{longtable}[!htbp]{|C{2.5em}L{11em}L{11em}C{2.5em}C{4em}C{5em}L{2.5em}|}
	...
	\rowcolor[rgb]{ .94,  .94,  .94}  3	&Predvolenej VLAN je povolené prepnutie na akýkoľvek port, VLAN hopping, double tagging	&Odobrať všetky porty z~predvolenej VLAN	&C	&C	&DI, CDA, AC	& \cite{uYLsMtQInofenpV3}\\
...
\begin{longtable}[!htbp]{|C{2.5em}L{11em}L{11em}C{2.5em}C{4em}C{5em}L{2.5em}|}
	...
	\rowcolor[rgb]{ .94,  .94,  .94} 11	&Možnosť urobiť rozdiel zmien konfigurácií a jej návrat	&Periodické zálohovanie konfigurácie a logovanie jej zmien	&M	&M	&A	& \cite{McMillan2018}
